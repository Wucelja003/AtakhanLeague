...
export default function LightPillar({
  ...
  // Only run the shader while the section is on screen + tab visible.
  ...
  useEffect(() => {
    ...
    // One static frame so it isn't blank before it scrolls into view.
    renderer.render(scene, camera);

    // Run only while visible (this lives on the home page, mostly off-screen).
    const io = new IntersectionObserver(
      (entries) => {
        const onScreen = entries[0]?.isIntersecting ?? false;
        runningRef.current = onScreen && !document.hidden && !reducedMotion;
      },
      { threshold: 0 },
    );
  ...
  return (
    <div ref={containerRef} aria-hidden="true" className={`w-full h-full absolute top-0 left-0 ${className}`} style={{ mixBlendMode }} />
  );
}
